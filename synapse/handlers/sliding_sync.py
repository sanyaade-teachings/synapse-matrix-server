...
class SlidingSyncRoomStore:
    """In-memory store of what rooms we have previously sent down a sliding sync
    connection.
    ...
    def have_sent_room(
        self, user_id: str, conn_id: str, store_token: int, room_id: str
    ) -> HaveSentRoom:
        ...
        room_status = sync_statuses.get(store_token, {}).get(
    ...
    def record_rooms(
        ...
        Attributes:
            user_id
            conn_id
            prev_store_token: The token from the request
            last_room_token: The room token from the request's pos token, i.e.
                this is
            new_sent_room_ids: The set of room IDs that we have sent down as
                part of this request (only needs to be ones we didn't
                previously sent down).
            new_missing_room_ids: The set of room IDs that have had updates
                since the `last_room_token`, but which were not included in
                this request
        """
        # If there are no changes then this is a noop.
        if not new_sent_room_ids and not new_missing_room_ids:
            return prev_store_token

        sync_statuses = self._connections.get((user_id, conn_id), {})

        # Generate a new token, removing any existing entries in that token
        # (which can happen if requests get resent).
        new_store_token = prev_store_token + 1
        ...
        for room_id in new_missing_room_ids:
            # Whether we add/update the entry for the room depends on the
            # existing entry:
            #   - LIVE: We have previously sent down everything up to
            #     `last_room_token, so we update the entry to be `PREVIOUSLY`
            #     with `last_room_token`.
            #   - PREVIOUSLY: We have previously sent down everything up to *a*
            #     given token, so we don't need to update the entry.
            #   - NEVER: We have never previously sent down the room, and we
            #     haven't sent anything down this time either so we leave it as
            #     NEVER.
            prev_state = new_room_statuses.get(room_id)
            if prev_state is not None and prev_state.status == HaveSentRoomFlag.LIVE:
                new_room_statuses[room_id] = previously_state
                have_updated = True
    ...
    def mark_token_seen(self, user_id: str, conn_id: str, store_token: int) -> None:
